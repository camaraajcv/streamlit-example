...
st.markdown(
    "<h1 style='text-align: center; font-size: 1.5em;'>DIRETORIA DE ADMINISTRAÇÃO DA AERONÁUTICA</h1>",
    unsafe_allow_html=True
)
st.markdown(
    "<h2 style='text-align: center; font-size: 1.2em;'>SUBDIRETORIA DE PAGAMENTO DE PESSOAL</h2>",
    unsafe_allow_html=True
)
st.markdown(
    "<h3 style='text-align: center; font-size: 1em; text-decoration: underline;'>PP1 - DIVISÃO DE DESCONTOS</h3>",
    unsafe_allow_html=True
)
...
def processar_pdf(pdf_content):
    ...
    # Use a função str.replace() para remover ".", "/" e "-" da coluna 'CNPJ'
    df['CNPJ'] = (
        df['CNPJ']
        .str.replace('.', '')
        .str.replace('/', '')
    ...
    # Filtrar o DataFrame
    df_rubricas_excluidas = df[
        df['Rubrica'].isin(valores_para_filtrar)
    ][['Rubrica', 'Empresa', 'Valor Líquido']]
    ...
    # Calcula a diferença entre a soma da coluna e o valor extraído
    ...
    df_final = df_final[
        ~df_final['Rubrica'].isin(valores_para_filtrar)
    ]

    valor_liquido_ajustado = round(
        df_final['Valor Líquido'].sum(),
        2
    )

    st.success(
        f"Valor Líquido sem as Rubricas excluídas: "
        f"{formatar_moeda(round(df_final['Valor Líquido'].sum(), 2))}"
    )
    ...
    with st.form(key='my_form'):
        ...
        with col1:
            ...
            numero_ne = st.text_input(
                "Número da NE: :red[(PARA FL)]",
                value=str(ano_atual) + 'NE',
                max_chars=12,
                key='numero_ne'
            )

            numero_sb = st.text_input(
                "Número do Subelemento: :red[(PARA FL)]",
                max_chars=2,
                key='numero_sb'
            )

            numero_fl = st.text_input(
                "Número da FL:",
                max_chars=6,
                key='numero_fl'
            )

            ano_empenho = st.text_input(
                "Ano de Referência (4 dígitos): :red[(PARA FL)]",
                value=str(ano_atual),
                max_chars=4,
                key='ano_empenho'
            )

            sequencial_fl = st.text_input(
                "Número Sequencial da FL: :red[(PARA FL)]",
                max_chars=4,
                key='sequencial_fl'
            )

            texto_obs = st.text_input(
                "Texto Observação:",
                value='DESC.EXT.CV',
                key='texto_obs'
            )

            mes_referencia_cc = st.text_input(
                "Número Mês Referência CC : :red[(PARA FL)]",
                value=str(mes_atual),
                max_chars=2,
                key='mes_referencia_cc'
            )

        # Coluna 2
        with col2:

            cpf_responsavel = st.text_input(
                "CPF do Responsável:",
                max_chars=11,
                key='cpf_responsavel'
            )

            data_previsao_pagamento = st.date_input(
                "Data de Previsão de Pagamento - (1° dia útil mês seguinte)",
                key='data_previsao_pagamento'
            )
            ...
            sequencial_deducao = st.text_input(
                "Número Sequencial da Dedução:",
                max_chars=4,
                key='sequencial_deducao'
            )
            ...
            ano_referencia_cc = st.text_input(
                "Número Ano Referência CC : :red[(PARA FL)]",
                value=str(ano_atual),
                max_chars=4,
                key='ano_referencia_cc'
            )
...
def exportar_xml(
    ...
    for seq_item, (index, row) in enumerate(
        df_final.iterrows(),
        start=1
    ):

        # Definição de codTipoOB, conta, banco_fab e txtCit
        # de acordo com os CNPJs específicos
        ...
        # ==========================================================
        # REGRA ESPECIAL PARA BANCO 104 + CONTA FOPAG
        #
        # Sempre que:
        #     Banco = 104
        #     Conta utilizada no XML = FOPAG
        #
        # a agência será obrigatoriamente 0231,
        # independentemente da agência informada no PDF.
        # ==========================================================

        banco_xml = str(row['BCO']).strip()

        # Determina qual conta será efetivamente utilizada no XML
        if codTipoOB == 'OBF':
            conta_xml = str(conta).strip()
        else:
            conta_xml = str(row['Conta']).strip()

        # Determina a agência
        if (
            banco_xml == '104'
            and conta_xml.upper() == 'FOPAG'
        ):
            agencia_xml = '0231'
        else:
            agencia_xml = str(row['AG']).strip()

        # Construção do domicílio bancário do favorecido
        numDomiBancFavo = f"""<numDomiBancFavo>
                                <banco>{banco_xml}</banco>
                                <agencia>{agencia_xml}</agencia>
                                <conta>{conta_xml}</conta>
                            </numDomiBancFavo>"""
        ...
        # Somente incluir a tag <txtCit> se codTipoOB for 'OBF'
        # e txtCit não for None
    ...
    xml_content_modelo2 = xml_content_modelo2.replace(
        'linhas_totais',
        str(len(df_final))
    )
    ...
    xml_content = re.sub(
        r'<txtCit>None</txtCit>',
        '',
        xml_content
    )

    xml_content_modelo2 = re.sub(
        r'<txtCit>None</txtCit>',
        '',
        xml_content_modelo2
    )
    ...
    st.success(
        "Arquivo XML com DataFrame gerado com sucesso."
    )
    ...
    st.download_button(
        ...
        file_name=(
            f"xml_FL_"
            f"{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.xml"
        ),
        mime="text/xml"
    )

    # Botão de download para DEDUÇÃO
    st.download_button(
        label="Baixar XML para DEDUÇÃO",
        data=io.BytesIO(
            xml_content_modelo2.encode()
        ),
...
uploaded_file = st.file_uploader(
    "Faça o UPLOAD do arquivo PDF do SIAPE gerado na transação GRCOCGRECO",
    type="pdf"
)
