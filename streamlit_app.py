...
def processar_pdf(pdf_content):
    ...
    # Localizar o valor líquido no texto usando uma expressão regular
    valor_liquido_pattern = r'Valor líquido\s*:\s*R\$\s*([\d\.,]+)'
    match = re.search(valor_liquido_pattern, text, re.IGNORECASE | re.DOTALL)
    ...
    # Converta as colunas para o tipo float
    ...
    with st.form(key='my_form'):
        ...
        with col1:
            numero_ne = st.text_input("Número NE")
            cpf_responsavel = st.text_input("CPF Responsável")

        with col2:
            numero_sb = st.text_input("Número SIAFI/SB")
            data_previsao_pagamento = st.date_input("Data Previsão Pagamento")

        with col3:
            ano_empenho = st.text_input("Ano Empenho")
            valor_liquido = st.text_input("Valor Líquido")

        with col4:
            data_vencimento = st.date_input("Data Vencimento")
